...
function friendlyError(error) {
  ...
  if (msg.includes("no_reset_pending")) return "There's no pending PIN reset — request a new code.";
  if (msg.includes("invalid_email_format")) return "Enter a valid email address.";
  if (msg.includes("email_taken")) return "That email is already registered to another account.";
  if (msg.includes("no_email_on_file")) return "This account doesn't have an email on file yet, so it can't be reset this way. Contact support.";
  if (msg.includes("email_mismatch")) return "That email doesn't match what's on file for this phone number.";
  if (msg.includes("invalid_code")) return "That code is incorrect.";
  return msg;
}
