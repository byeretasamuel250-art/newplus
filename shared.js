...
function safeRender(fn) {
  return async (...args) => {
    try {
      ...
      // otherwise a dropped connection mid-request (e.g. Safari's
      // "TypeError: Load failed") surfaces as a raw JS error string instead
      // of a message a user can act on. This is the same helper every
      // handled RPC error already goes through; friendlyError() just passes
      // through anything it doesn't specifically recognize (still no worse
      // than before), except this class of generic network failure.
      root.innerHTML += `<div class="error-banner"><strong>Something went wrong.</strong><br>${escapeHtml(friendlyError(err))}</div>`;
    }
  };
}
...
function friendlyError(error) {
  ...
  // Catch those here so a raw JS error string never surfaces to the user;
  // everywhere that calls friendlyError() (including safeRender()'s
  // catch-all below) gets this same friendly wording instead.
