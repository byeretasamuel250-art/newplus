...
**If you already ran schema.sql before:** this update adds two new
notification types — a top-level comment on your post and a like on your
post now notify you instantly too, not just replies to your own comment.
Just paste the whole `schema.sql` into the SQL Editor and run it again —
everything uses `create or replace` / `if not exists` / `add column if
not exists`, so nothing existing gets touched or lost.
