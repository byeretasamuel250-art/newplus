...
-- Bypass RLS (via security definer) to look up a comment's parent — used by
-- the status_comments policies below. A policy on status_comments cannot
-- safely query status_comments directly (Postgres reports "infinite
-- recursion detected in policy"), so any self-referential lookup has to go
-- through a function like this instead.
create or replace function status_comment_parent_profile(p_comment_id uuid)
returns uuid
language sql security definer stable
as $$
  select profile_id from status_comments where id = p_comment_id;
$$;
grant execute on function status_comment_parent_profile(uuid) to anon, authenticated;
...
begin
  ...
  return jsonb_build_object('ok', true, 'profile', to_jsonb(match_row));
end;
$$;
...
-- Comments stay private feedback to the poster, not a public thread — the
-- status owner (or admin) reads the whole thread back — but a reply is
-- only useful if the person it's aimed at can see it, so a commenter can
-- also see their own comments and any reply made to one of their own
-- comments (e.g. the post owner replying to them). A comment can be
-- removed by whoever wrote it, the post's owner (moderation on your own
-- post), or admin.
...
create policy "status_comments_select_owner_author_or_reply_target" on status_comments for select
  using (
    is_admin()
    or exists (
    ...
    or status_comment_parent_profile(status_comments.reply_to_id) = my_profile_id()
  );
