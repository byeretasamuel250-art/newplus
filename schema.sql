...
-- migrate existing deployments: drop the parent-profile lookup from the
-- earlier version of this feature — no longer needed now that only the
-- post owner (and admin) can read the comment thread back
