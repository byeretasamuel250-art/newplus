...
create policy "status_comments_select_owner_author_or_reply_target" on status_comments for select
  using (
    ...
    or exists (
      select 1 from status_comments parent
      where parent.id = status_comments.reply_to_id
        and parent.profile_id = my_profile_id()
    )
  );
    ...
    and (
      ...
      or exists (
        ...
        where parent.id = reply_to_id
          and parent.status_id = status_comments.status_id
      )
    )
  );
