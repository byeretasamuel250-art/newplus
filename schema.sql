...
-- notifications — currently just "someone replied to your comment", created
-- automatically by the trigger below (never inserted directly by clients)
create table if not exists notifications (
  ...
  type text not null default 'comment_reply' check (type in ('comment_reply')),
...
-- Automatically notify the author of a comment when someone replies to it
-- (skips notifying yourself if you reply to your own comment). Runs as the
-- function owner (security definer), so it can write to `notifications`
-- regardless of the inserting user's own RLS access to that table — same
-- pattern as the helper functions above.
...
begin
  ...
  update profiles set pin_hash = encode(digest(p_new_pin, 'sha256'), 'hex'), pin_must_change = false where id = me.id;
end;
$$;
...
create or replace function request_pin_reset(p_phone text)
returns jsonb
language plpgsql security definer
as $$
declare
...
begin
  ...
  -- idempotent: if one's already pending, don't error, just leave it
  if not exists (select 1 from pin_reset_requests where profile_id = target.id and status = 'pending') then
    insert into pin_reset_requests (profile_id, phone) values (target.id, p_phone);
  end if;
  return jsonb_build_object('ok', true);
end;
$$;
grant execute on function request_pin_reset(text) to anon, authenticated;
...
begin
  ...
  update profiles
    set pin_hash = encode(digest(p_temp_pin, 'sha256'), 'hex'),
