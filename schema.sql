...
declare
  me uuid;
  new_msg messages;
  require_subscription boolean := true; -- flip to true later to require an active subscription before people can chat
begin
  select id into me from profiles where auth_uid = auth.uid();
  if me is null then raise exception 'no matching profile'; end if;
  if not exists (
...
-- ============================================================
-- SECURITY HARDENING — closes gaps where the app's own rules
-- (age gate, PIN hashing, generated file paths) were only enforced by the
-- client-facing functions, not by the database itself. Without this, a
-- technical user could bypass the app entirely and call the Supabase REST
-- API directly (with nothing more than the public anon key every browser
-- already has) to write straight to the `profiles` table, skipping every
-- check `register_with_pin` / `update_profile_details` normally does.
-- Re-running this file is always safe.
-- ============================================================

-- current_user is 'anon' or 'authenticated' for a request that hits the
-- table directly (that's the role PostgREST switches to based on the
-- caller's JWT). Once execution enters one of this file's SECURITY DEFINER
-- functions (register_with_pin, login_with_pin, change_pin,
-- admin_set_temp_pin, set_new_pin_after_reset, etc.), current_user changes
-- to that function's owner instead — so this check reliably tells "came
-- through one of our vetted functions" apart from "a client called the
-- REST API directly", without needing to know or guess the owner's exact
-- role name.
create or replace function is_direct_client_call()
returns boolean
language sql stable
as $$
...
create or replace function protect_profile_integrity()
returns trigger
language plpgsql
as $$
begin
  -- A profile row must only ever be created by register_with_pin() — that
  -- function applies the 18+ gate and hashes the PIN before inserting.
  -- A direct INSERT would skip both.
  if TG_OP = 'INSERT' and is_direct_client_call() then
    raise exception 'profiles must be created via register_with_pin()';
  end if;

  if TG_OP = 'UPDATE' and is_direct_client_call() then
    -- pin_hash and phone may only change through change_pin(),
    -- login_with_pin()'s bcrypt upgrade, admin_set_temp_pin(),
    -- set_new_pin_after_reset(), or register_with_pin() — never by a
    -- direct client write, which could otherwise set an unhashed pin_hash
    -- or swap in an arbitrary phone number outside the registration flow.
  ...
  -- Age gate, enforced here too (not just in register_with_pin /
  -- update_profile_details) so a direct client write can never set an
  -- under-18 date of birth either.
  if new.dob is not null and new.dob > (current_date - interval '18 years') then
    raise exception 'must_be_18';
  end if;

  -- avatar_path must look exactly like what the app itself generates —
  -- {this profile's own id}/{timestamp}.jpg — same reasoning as the
  -- image_path / voice path checks already in send_message(): this value
  -- gets built into an <img src="..."> for every other user who views this
  -- profile (directory, chat header, status ring), so it must never be
  -- attacker-controlled free text.
  if new.avatar_path is not null
     and new.avatar_path !~ ('^' || new.id::text || '/[0-9]+\.jpg$') then
...
drop trigger if exists trg_protect_profile_integrity on profiles;
create trigger trg_protect_profile_integrity
  before insert or update on profiles
  for each row execute function protect_profile_integrity();

-- ------------------------------------------------------------
-- Phone-number probing throttle
-- ------------------------------------------------------------
-- register_with_pin() ("phone_taken") and request_pin_reset()
-- ("phone_not_found" vs "ok") both tell an anonymous caller whether a
-- given phone number has an account. Neither needs a login, so without a
-- limit here a script could loop through phone numbers to find out who's
-- on the app. This caps how many *different* phone numbers a single
-- anonymous session can probe per rolling hour across both functions.
-- (It can't stop someone from opening many separate anonymous sessions,
-- but it closes off the cheap version of this — one session, many
-- numbers, in seconds — and Supabase's own anonymous-sign-in rate limit
-- adds further friction on top.)
create table if not exists phone_probe_attempts (
  auth_uid uuid primary key,
  probe_count int not null default 0,
  window_started_at timestamptz not null default now()
);
-- no policies created on purpose: RLS enabled + zero policies means no
-- client (anon or authenticated) can read or write this table directly,
-- same pattern as login_attempts above. Only touched via the function
-- below (security definer, so it bypasses RLS).
alter table phone_probe_attempts enable row level security;

create or replace function check_phone_probe_limit()
returns void
language plpgsql security definer
as $$
declare
  uid uuid := auth.uid();
  row_data phone_probe_attempts;
begin
  if uid is null then return; end if; -- caller functions already require a session

  select * into row_data from phone_probe_attempts where auth_uid = uid;

  if row_data is null then
    insert into phone_probe_attempts (auth_uid, probe_count, window_started_at)
    values (uid, 1, now());
    return;
  end if;

  if row_data.window_started_at < now() - interval '1 hour' then
    update phone_probe_attempts
      set probe_count = 1, window_started_at = now()
      where auth_uid = uid;
    return;
  end if;

  if row_data.probe_count >= 20 then
    raise exception 'too_many_attempts';
  end if;

  update phone_probe_attempts set probe_count = probe_count + 1 where auth_uid = uid;
end;
$$;

-- Wire the throttle into both phone-lookup entry points. Both are
-- recreated with the exact same signature/behavior as before, plus one
-- call to check_phone_probe_limit() at the top.
create or replace function register_with_pin(p_phone text, p_pin text, p_dob date, p_district text)
returns jsonb
language plpgsql security definer
as $$
...
begin
  if auth.uid() is null then raise exception 'no active session'; end if;
  perform check_phone_probe_limit();
  if exists (select 1 from profiles where phone = p_phone) then raise exception 'phone_taken'; end if;
  if p_dob is null or p_dob > (current_date - interval '18 years') then
    raise exception 'must_be_18';
  end if;
...
-- ------------------------------------------------------------
-- Ad links: restrict to http(s) so a compromised admin account (or a
-- typo) can't plant a javascript:/data: URL that runs in every user's
-- browser when they tap a banner ad.
-- ------------------------------------------------------------
alter table ads drop constraint if exists chk_ads_link_scheme;
alter table ads add constraint chk_ads_link_scheme
