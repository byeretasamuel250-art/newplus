...
-- per-side mute: when true, that participant does not get push
-- notifications for new messages in this conversation (chat still
-- works normally otherwise — this only affects notifications).
alter table conversations add column if not exists muted_a boolean not null default false;
alter table conversations add column if not exists muted_b boolean not null default false;
...
begin
  -- For messages, check whether the recipient (the participant who is
  -- NOT the sender) has muted this specific conversation — if so, skip
  -- the push entirely. Notifications (comment replies, likes, etc.)
  -- aren't tied to a conversation, so this check doesn't apply to them.
  if TG_TABLE_NAME = 'messages' then
    select case
      when user_a = NEW.sender_id then muted_b
      when user_b = NEW.sender_id then muted_a
      else false
    end into recipient_muted
...
-- Lets a user mute (or unmute) push notifications for one specific
-- conversation, without affecting any other conversation or their
...
begin
  ...
  if me is null then raise exception 'no matching profile'; end if;
  update conversations set
    muted_a = case when user_a = me then p_muted else muted_a end,
    muted_b = case when user_b = me then p_muted else muted_b end
  where id = p_conversation_id and (user_a = me or user_b = me);
end;
$$;
grant execute on function set_conversation_muted(uuid, boolean) to anon, authenticated;
