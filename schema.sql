...
-- migrate existing deployments: lets a sender edit a text message they
-- already sent, tracked so the chat can show an "Edited" marker
...
-- lets a user edit the text body of a message they sent — only their own
-- messages, only text ones (photos/voice notes can't be edited), same
-- security-definer pattern as send_message() so no separate update policy
-- is needed on the messages table.
...
begin
  select id into me from profiles where auth_uid = auth.uid();
  if me is null then raise exception 'no matching profile'; end if;
  if p_new_body is null or length(trim(p_new_body)) = 0 or length(p_new_body) > 4000 then
    raise exception 'invalid_message_body';
  end if;
  update messages
  set body = p_new_body, edited_at = now()
  where id = p_message_id and sender_id = me and kind = 'text'
