...
self.addEventListener("fetch", (event) => {
  ...
  // Photos are requested two different ways for the same file: an <img>
  // tag (no-cors, opaque response — used just to display it) and, for the
  // "Save image" feature, an explicit page-level fetch() (cors, readable
  // response — needed so that code can read res.ok and turn it into a
  // downloadable blob). An opaque response can never become readable
  // later no matter how it's re-requested, so caching it under the same
  // key as a cors request would make "Save image" permanently fail once
  // a photo had already been viewed (res.ok reads false forever on a
  // served opaque response). Keeping the two modes in separate cache
  // entries avoids that — this only costs an extra download the first
  // time a *previously-viewed* photo is saved, never after.
  const modeTag = req.mode === "no-cors" ? "img" : "fetch";
  const cacheKey = new Request(url.origin + url.pathname + "?__swmode=" + modeTag, { method: "GET" });
